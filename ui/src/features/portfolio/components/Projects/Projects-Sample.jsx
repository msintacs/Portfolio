// ui/src/pages/portfolio/components/Projects/Projects.jsx
import React, { useState } from "react";
import { projectsDataSample } from "./PrjectsData-Sample";

function ProjectsSample() {
  const [activeTab, setActiveTab] = useState(projectsDataSample.projects[0].id);

  return (
    <div className="w-full bg-white py-16">
      <div className="mx-auto max-w-[1200px] px-4 sm:px-6 lg:px-8">
        <h2 className="mb-2 text-center font-SUIT-Heavy text-4xl font-bold">
          {projectsDataSample.title}
        </h2>
        <p className="mx-auto mb-10 max-w-2xl text-center text-lg text-gray-600">
          {projectsDataSample.subTitle}
        </p>

        {/* Project Tabs */}
        <div className="mb-12 flex flex-wrap justify-center gap-2">
          {projectsDataSample.projects.map((project) => (
            <button
              type="button"
              key={project.id}
        ...
        {projectsDataSample.showMoreLink && (
          <div className="mt-12 text-center">
            <a
              href={projectsDataSample.showMoreLink}
              target="_blank"
              rel="noopener noreferrer"
